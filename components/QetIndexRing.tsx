  ...
  return (
    <div className="flex flex-col items-center">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {/* Äußerer Ring: 7 Managementfelder. Jedes Segment zeigt den
         * erreichten Anteil zentrisch in Grün: die grüne Linie liegt auf
         * derselben Mittellinie wie der graue Track, wird aber mit
         * wachsender Strichstärke gezeichnet, je höher der Wert – bei
         * 100 % deckt sie den grauen Rand vollständig ab. Ein dunkler
         * Rahmen je Segment macht die volle (=100 %) Breite sichtbar, damit
         * der Füllstand ablesbar bleibt. */}
        {fields.map((field, i) => {
          const start = i * (fieldSweep + fieldGap);
          const end = start + fieldSweep;
          const labelId = `field-pct-${field.key}`;
          const filledWidth = fieldRingWidth * (field.score / 100);
          return (
            <g key={field.key}>
              <title>{`${field.name[locale]}: ${Math.round(field.score)}%`}</title>
              <path
                d={ringArcPath(cx, cy, outerR3, start, end)}
                fill="none"
                stroke={SEGMENT_BORDER_COLOR}
                strokeWidth={fieldRingWidth + segmentBorderWidth * 2}
                strokeLinecap="round"
              />
              <path
                d={ringArcPath(cx, cy, outerR3, start, end)}
              ...
              {filledWidth > 0.5 && (
                <path
                  ...
                  fill="none"
                  stroke={QET_GREEN}
                  strokeWidth={filledWidth}
                  strokeLinecap="round"
                />
              )}
              <path id={labelId} d={labelArcPath(cx, cy, outerR3, start + 2.5, end - 2.5)} fill="none" />
              <text fontSize={fieldPctFontSize} fontWeight={700} fill="#ffffff" letterSpacing={0.3}>
        ...
        {/* Mittlerer Ring: 3 Säulen, Farbfüllung nach erreichtem Anteil */}
        {pillarSegments.map((seg) => {
          ...
          return (
            <g key={seg.key}>
              ...
              <path
                d={ringArcPath(cx, cy, outerR2, seg.start, seg.start + pillarSweep)}
                fill="none"
                stroke={SEGMENT_BORDER_COLOR}
                strokeWidth={pillarRingWidth + segmentBorderWidth * 2}
                strokeLinecap="round"
              />
              <path
                d={ringArcPath(cx, cy, outerR2, seg.start, seg.start + pillarSweep)}
                fill="none"
                stroke={TRACK_COLOR}
                strokeWidth={pillarRingWidth}
                strokeLinecap="round"
              />
              {filledSweep > 0.5 && (
                <path
                  d={ringArcPath(cx, cy, outerR2, seg.start, seg.start + filledSweep)}
                  fill="none"
                  stroke={PILLAR_COLOR[seg.key]}
                  strokeWidth={pillarRingWidth}
                  strokeLinecap="round"
                />
              )}
