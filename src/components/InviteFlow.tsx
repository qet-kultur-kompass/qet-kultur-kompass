  ...
  useEffect(() => {
    ...
    fetch(`/api/invite/${token}`)
      ...
      .then((data: InviteInfo) => {
        if (cancelled) return;
        setInfo(data);
        if (data.alreadyCompleted) setStage("already_done");
        else if (data.requiresSso) setStage("sso_gate");
        else setStage("intro");
      })
      .catch(() => {
    ...
    return (
      <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-6 text-ink/70">
          Diese Einladung für <strong>{info.companyName}</strong> wurde bereits beantwortet. Vielen
